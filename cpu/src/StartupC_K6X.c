...
void start(void)                              //根据飞思卡尔提供标准启动程序修改
{
...
#if (defined(MK60D10)) 
  MCG_C5 = MCG_C5_PRDIV0(OCXOPRDIV);                              //分频， EXTAL_IN_MHz/( PRDIV+1)
  MCG_C6 = MCG_C6_PLLS_MASK | MCG_C6_VDIV0(OCXOVDIV) ;             //倍频， EXTAL_IN_MHz/( PRDIV+1)  * (VDIV+16)/2
#else 
...
#elif (defined(M60DZ10) || defined(M60F15) || defined(M60D10))
  while (!(MCG_S & MCG_S_LOCK_MASK));
   ...
   SysClock.core_mhz = CORECLK;                                        //记录时钟以供调用
   SysClock.core_khz = CORECLK * 1000;
   SysClock.bus_khz = SysClock.core_khz / (((SIM_CLKDIV1 & SIM_CLKDIV1_OUTDIV2_MASK) >> 24) + 1);

   
#if (defined(MK60F15) || defined(MK66F18)) 
   SCB->CPACR |=((3UL << 10*2)|(3UL << 11*2));                     ////开启硬件浮点且需要同时打开IAR选项
#endif
   ////////////////////执行用户函数
   main();
   while(1); /////////死循环
